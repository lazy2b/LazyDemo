...
import com.lazy2b.libs.utils.BezierEvaluator;
...
//    }
    ...
    private void init() {
        ...
        dHeight = 50;//red.getIntrinsicHeight();
        dWidth = 50;//red.getIntrinsicWidth();
    ...
    public void setDrawables(int[] drawIds) {
        try {
            //初始化显示的图片
            drawables = new Drawable[drawIds.length];

            for (int i = 0; i < drawIds.length; i++) {
                drawables[i] = getResources().getDrawable(drawIds[i]);
            }

            //获取图的宽高 用于后面的计算
            //注意 我这里3张图片的大小都是一样的,所以我只取了一个
            dHeight = drawables[0].getIntrinsicHeight();//red.getIntrinsicHeight();
            dWidth = drawables[0].getIntrinsicWidth();//red.getIntrinsicWidth();
        } catch (Exception ex) {
    ...
    public void addHeart() {
        ...
        if (drawables == null) {
            ...
            drawables[1] = yellow;
            drawables[2] = blue;
            dHeight = red.getIntrinsicHeight();//red.getIntrinsicHeight();
            dWidth = red.getIntrinsicWidth();//red.getIntrinsicWidth();
        }
    ...
    private synchronized void addIcon(Drawable draw) {

        try {
            dHeight = draw.getIntrinsicHeight();//red.getIntrinsicHeight();
            dWidth = draw.getIntrinsicWidth();//red.getIntrinsicWidth();
            //底部 并且 水平居中
            LayoutParams lp= new LayoutParams(dWidth, dHeight);
            lp.addRule(CENTER_HORIZONTAL, TRUE);//这里的TRUE 要注意 不是true
            lp.addRule(ALIGN_PARENT_BOTTOM, TRUE);
    ...
    public void addIcon(int dId) {
        try {
            addIcon(getResources().getDrawable(dId));
    ...
    public void addIcon() {
        ...
        try {
            if (drawables != null && drawables.length > 0) {
                addIcon(drawables[random.nextInt(drawables.length)]);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
